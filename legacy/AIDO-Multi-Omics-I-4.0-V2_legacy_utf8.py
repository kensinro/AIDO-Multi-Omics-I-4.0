...
Internal benchmark pipeline v2 FIXED for NARGAB upgrade

Major fixes in v2
-----------------
1. Robust UTF-8 / UTF-16 / UTF-8-SIG / Latin1 input reader.
2. Clinical patient-ID suffix remapping:
   e.g. A0C8 -> TCGA-DK-A0C8 when uniquely matched to molecular IDs.
3. Burden evaluation fixed:
   high_burden is evaluated as a binary group directly.
   It is NOT median-split again.
4. PRAD / no-stage / low-event cohorts handled safely.
5. Outputs all summary files to D:/AIDO-Temp/.
...
Output
------
D:/AIDO-Temp/AIDO_MultiOmics_I_4_internal_benchmark_v2_FIXED_<timestamp>/
"""
...
OUT_DIR = OUT_ROOT / f"AIDO_MultiOmics_I_4_internal_benchmark_v2_FIXED_{TIMESTAMP}"
...
def tcga_patient_id(x: Any) -> Optional[str]:
    """
    Standard TCGA patient barcode parser.
    If a string contains full TCGA-XX-XXXX, return first 12 chars.
...
def read_table_auto(path: Path, index_col: Optional[int] = None, nrows: Optional[int] = None) -> pd.DataFrame:
    """
    Robust reader for UTF-8 / UTF-8-SIG / UTF-16 / UTF-16-LE / Latin1.
    Most UCSC Xena files are TSV, but separator is also tested.
    """
...
def normalize_matrix_gene_by_patient(df: pd.DataFrame, layer: str) -> pd.DataFrame:
    """
    Convert TCGA/Xena-style molecular file into genes x patients.

    Supports:
    - GE/CN gene x samples matrix.
    - MU gene x samples matrix.
    - MU long table with sample and gene columns.
    """
    df = df.copy()
...
# =============================================================================
# 5. CLINICAL PARSING + ID REMAPPING FIX
# =============================================================================
...
def remap_clinical_ids_to_molecular(clinical: pd.DataFrame, molecular_patients: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Fix clinical IDs like A0C8, 3306, 1592 by matching them to molecular TCGA IDs:
    A0C8 -> TCGA-DK-A0C8 if unique.

    Rules:
    - Full TCGA IDs are kept.
    - Non-full IDs are matched to the last segment of molecular IDs.
    - Only unique suffix matches are used.
    - Ambiguous suffixes are not remapped.
    """
    ...
    for pid in molecular_patients:
        ...
        if len(parts) >= 3:
            suffixes.append(parts[-1])              # A0C8
            suffixes.append("-".join(parts[-2:]))   # DK-A0C8
        suffixes.append(pid)
    ...
    for old_id in clinical.index:
        ...
        if old in molecular_patients:
            ...
            # Also try last 4 chars if old has extra clutter
    ...
    # If duplicates after remapping, keep the first non-null rich row.
...
def load_clinical(files: Dict[str, Optional[Path]], molecular_patients: Optional[List[str]] = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for key in ["PHENO", "CLINICAL", "STAGE_GROUP"]:
        ...
        if p is not None and p.exists():
            try:
                ...
                dfs.append(d)
                source_rows.append({"source": key, "file": str(p), "n_rows": d.shape[0], "n_cols": d.shape[1]})
    ...
    # Prefer direct death/follow-up fields if available.
...
def survival_test_from_score(score: pd.Series, clinical: pd.DataFrame) -> Dict[str, Any]:
    """
    Score-based endpoint test.
    Used for representation-level BP score evaluation.
    This function median-splits continuous scores.
    """
...
def survival_test_from_group(group: pd.Series, clinical: pd.DataFrame) -> Dict[str, Any]:
    """
    FIXED group-based OS test.
    Used for high_burden group.
    Does NOT median-split again.
    """
    group = group.dropna()
...
def stage_test_from_group(group: pd.Series, clinical: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Binary-group AUC approximation
    AUC = np.nan
    try:
...
def select_best_representations(rep_map: pd.DataFrame) -> pd.DataFrame:
    ...
    if rep_map.empty or not {"cohort", "bp", "endpoint", "representation", "D"}.issubset(rep_map.columns):
        return pd.DataFrame()
...
def construct_bp_burden(
    ...
    # q67 can equal max in sparse data; still keep, but group test will require 2 groups.
...
def evaluate_burden(burden_df: pd.DataFrame, clinical: pd.DataFrame) -> Dict[str, Any]:
    """
    FIXED burden evaluation.
    Uses high_burden as binary group directly.
    """
...
def process_one_cohort(
    ...
    # Load molecular first so we can remap clinical IDs using molecular patient IDs.
    layer_mats = {}
    ...
    write_csv(clinical.reset_index().rename(columns={"index": "patient"}), cohort_out / f"{cohort}_clinical_parsed_FIXED.csv")

    if not clinical_mapping.empty:
        write_csv(clinical_mapping, cohort_out / f"{cohort}_clinical_id_mapping_FIXED.csv")
    ...
    if len(matched_clinical_molecular) < 30:
        log(f"Skipping {cohort}: too few matched clinical-molecular patients")
    ...
    for layer in ["GE", "CN", "MU"]:
        try:
            ...
            log(f"Failed BP score construction {cohort} {layer}: {e}")
            return {"cohort": cohort, "status": f"skipped_failed_BP_scores_{layer}", "error": str(e)}
    ...
    if "GE" not in rep_scores:
        log(f"Skipping {cohort}: no GE rep score")
        return {"cohort": cohort, "status": "skipped_no_GE_rep_score"}
    ...
    write_csv(rep_map, cohort_out / f"{cohort}_representation_map_all_reps_FIXED.csv")
    write_csv(selected, cohort_out / f"{cohort}_selected_representations_deltaD_FIXED.csv")

    # K sensitivity burden
    burden_rows = []
    selected_bp_rows = []
        ...
        for endpoint in ENDPOINTS:
            for cutoff in ABNORMALITY_CUTOFFS:
                ...
                if not bdf.empty:
                    ...
                    write_csv(bdf, cohort_out / f"{cohort}_burden_{endpoint}_K{K}_{cutoff}_FIXED.csv")
    ...
    write_csv(burden_summary, cohort_out / f"{cohort}_burden_K_sensitivity_summary_FIXED.csv")

    if not selected_bp_summary.empty:
        write_csv(selected_bp_summary, cohort_out / f"{cohort}_selected_BP_for_burden_allK_FIXED.csv")

    # Random baseline, primary K only
    random_rows = []
    observed_lookup = burden_summary[burden_summary["K"] == PRIMARY_K].copy() if not burden_summary.empty else pd.DataFrame()
    ...
    for endpoint in ENDPOINTS:
        for cutoff in ABNORMALITY_CUTOFFS:
            ...
            if not rand_df.empty:
                write_csv(rand_df, cohort_out / f"{cohort}_random_baseline_{endpoint}_K{PRIMARY_K}_{cutoff}_FIXED.csv")
    ...
    write_csv(random_summary, cohort_out / f"{cohort}_random_baseline_summary_FIXED.csv")
    ...
    write_csv(split_df, cohort_out / f"{cohort}_split_validation_K{PRIMARY_K}_EXT20_FIXED.csv")
    ...
    write_json(status, cohort_out / f"{cohort}_status_FIXED.json")
...
def aggregate_outputs(out_dir: Path, cohort_status: pd.DataFrame) -> None:
    ...
    patterns = {
        "all_representation_maps": "*_representation_map_all_reps_FIXED.csv",
        "all_selected_representations": "*_selected_representations_deltaD_FIXED.csv",
        "all_burden_K_sensitivity": "*_burden_K_sensitivity_summary_FIXED.csv",
        "all_random_baseline_summary": "*_random_baseline_summary_FIXED.csv",
        "all_split_validation": "*_split_validation_K8_EXT20_FIXED.csv",
        "all_readiness": "*_readiness_all_layers.csv",
        "all_clinical_id_mapping": "*_clinical_id_mapping_FIXED.csv",
    }
...
def make_summary_tables(out_dir: Path) -> None:
    ...
    if selected_path.exists():
        ...
        rep_counts = df.groupby(["endpoint", "best_representation"]).size().reset_index(name="n")
        write_csv(rep_counts, agg / "summary_best_representation_counts.csv")

        class_counts = df.groupby(["endpoint", "representation_class"]).size().reset_index(name="n")
        write_csv(class_counts, agg / "summary_representation_class_counts.csv")

        gain_counts = df.groupby(["endpoint", "gain_class"]).size().reset_index(name="n")
        write_csv(gain_counts, agg / "summary_deltaD_gain_class_counts.csv")

        frag_counts = df.groupby(["endpoint", "fragility_class"]).size().reset_index(name="n")
        write_csv(frag_counts, agg / "summary_fragility_class_counts.csv")

        endpoint_info = df.groupby("endpoint")["endpoint_informative"].agg(["sum", "count"]).reset_index()
        endpoint_info["fraction"] = endpoint_info["sum"] / endpoint_info["count"]
        write_csv(endpoint_info, agg / "summary_endpoint_informative_counts.csv")

    if burden_path.exists():
        df = pd.read_csv(burden_path, encoding="utf-8-sig")
        if not df.empty:
            k_summary = df.groupby(["endpoint_selection", "K", "cutoff"]).agg(
            ...
            write_csv(k_summary, agg / "summary_K_sensitivity.csv")
    ...
    if random_path.exists():
        ...
        if not df.empty:
            ...
            write_csv(df, agg / "all_random_baseline_summary_with_flags.csv")

            rand_summary = df.groupby(["endpoint_selection", "cutoff"]).agg(
            ...
            write_csv(rand_summary, agg / "summary_random_baseline_flags.csv")

    if split_path.exists():
        df = pd.read_csv(split_path, encoding="utf-8-sig")
        if not df.empty:
            split_summary = df.groupby(["cohort", "endpoint_selection", "K", "cutoff"]).agg(
...
def make_simple_figures(out_dir: Path) -> None:
    ...
    if selected_path.exists():
        ...
        for endpoint in ENDPOINTS:
            ...
            sub2 = df[(df["endpoint"] == endpoint) & df["deltaD_full_minus_GE"].notna()]
            if not sub2.empty:
                plt.figure(figsize=(8, 5))
                plt.hist(sub2["deltaD_full_minus_GE"], bins=40)
                plt.axvline(0, linestyle="--")
                plt.xlabel("Delta D: full integration GE+CN+MU minus GE")
                plt.ylabel("Cancer-BP observations")
                plt.title(f"{endpoint}: Full integration gain/loss over GE")
                plt.tight_layout()
                plt.savefig(fig_dir / f"DeltaD_full_minus_GE_{endpoint}.png", dpi=300)
                plt.close()

    if random_path.exists():
        df = pd.read_csv(random_path, encoding="utf-8-sig")

        for metric in ["OS", "STAGE"]:
            obs_col = f"observed_{metric}_D"
            rand_col = f"random_{metric}_95pct"

            if obs_col not in df.columns or rand_col not in df.columns:
                continue
            ...
            if not sub.empty:
                ...
                plt.xlabel(f"Random 95th percentile {metric} D")
                plt.ylabel(f"Observed selected burden {metric} D")
                plt.title(f"All-cohort random baseline: {metric}")
                plt.tight_layout()
                plt.savefig(fig_dir / f"Random_baseline_observed_vs_random95_{metric}.png", dpi=300)
                plt.close()
...
def main() -> None:
    ...
    config = {
        ...
        "major_fixes": [
            "clinical suffix ID remapping",
            "group-based burden evaluation without second median split",
            "robust UTF-8/UTF-16 reader",
        ],
    }
    write_json(config, OUT_DIR / "run_config.json")

    log("AIDO-Multi-Omics-I-4.0 internal benchmark v2 FIXED started")
    ...
    if not HALLMARK_GMT.exists():
        raise FileNotFoundError(f"HALLMARK_GMT not found: {HALLMARK_GMT}")

    gene_sets = load_gmt(HALLMARK_GMT)
    log(f"Loaded gene sets: {len(gene_sets)}")
    ...
    log("AIDO-Multi-Omics-I-4.0 internal benchmark v2 FIXED finished")
